 ...
 function acajoombot_content_getitems() {
		if ( ACA_CMSTYPE ) {
			...
			global $database ;
			 ...
			 $query = "SELECT a.id as id, a.title as title, b.title as section, c.title as category FROM #__content as a LEFT JOIN #__sections as b ON a.sectionid = b.id LEFT JOIN #__categories AS c ON a.catid = c.id ORDER BY a.sectionid, a.catid, a.created DESC";
	 ...
	 foreach ($content_items as $content_item) {
		 ...
		 if ($content_item[2] == 0) {
			...
			if ( ACA_CMSTYPE ) {	// joomla 15
				 $link = JRoute::_( ACA_JPATH_LIVE.'/index.php?option=com_content&task=view&id='.$content_item[1].'&Itemid='.$Itemid );
			} else {									//joomla 1x
				 if ($GLOBALS[ACA.'use_sef'] == '1' AND $GLOBALS['mosConfig_sef'] == '1' AND function_exists('sefRelToAbs')) {
				 	...
				 	$link = ACA_JPATH_LIVE.'/index.php?option=com_content&task=view&id='.$content_item[1].'&Itemid='.$Itemid;
				 }
			}//endif
		 ...
		 foreach($images as $image) {
			 $image_string = '<img src="' . ACA_JPATH_LIVE . '/images/stories/' . $image['image'] . '" align="' . $image['align'] . '" alt="' . $image['alttext'] . '" border="' . $image['border'] . '" />';
	...
	$Itemid = $GLOBALS[ACA.'itemidAca'];
 	...
 	if(!empty($tags[0])){
 		foreach ($tags[0] as $tag){
			...
			if($parameters[3]){
				if ( ACA_CMSTYPE ) {	// joomla 15
					 $link = JRoute::_(ACA_JPATH_LIVE.'/index.php?option=com_jcalpro&extmode=view&extid='.$event->extid.'&Itemid='.$Itemid );
				} else {									//joomla 1x
		 			if ($GLOBALS[ACA.'use_sef'] == '1' AND $GLOBALS['mosConfig_sef'] == '1' AND function_exists('sefRelToAbs')) {
					 	$link = sefRelToAbs('index.php?option=com_jcalpro&extmode=view&extid='.$event->extid.'&Itemid='.$Itemid);
					 } else {
					 	$link = ACA_JPATH_LIVE.'/index.php?option=com_jcalpro&extmode=view&extid='.$event->extid.'&Itemid='.$Itemid;
					 }
				}//endif
