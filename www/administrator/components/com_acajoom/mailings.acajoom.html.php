	...
	<tr>
		...
		if ((!$show['admin']) OR ( $mailing->published == 1 AND ($mailing->list_type == 1 or $mailing->list_type == 7 or $mailing->list_type == 2))) {
			if ($show['index'] == 'index') {
				...
				if ( ACA_CMSTYPE ) {	// joomla 15
					$link = JRoute::_(ACA_JPATH_LIVE.'/'.$show['index'].'.php?option=com_acajoom&act=' . $action . '&task=view&listid='.$mailing->list_id.'&mailingid=' .$mailing->id . $item );
				} else {									//joomla 1x
			...
			if ($show['index'] == 'index') {
				...
				if ( ACA_CMSTYPE ) {	// joomla 15
					$link = JRoute::_(ACA_JPATH_LIVE.'/'.$show['index'].'.php?option=com_acajoom&act=' . $action . '&task=edit&mailingid=' .$mailing->id . '&listid=' . $mailing->list_id . '&listype=' .$mailing->list_type . $item );
				} else {									//joomla 1x
					if ( function_exists('sefRelToAbs') AND $GLOBALS[ACA.'use_sef'] ) {
