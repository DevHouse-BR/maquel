...
if ( !defined('ACA_JPATH_ROOT') AND class_exists('JFactory')) {
...
global $mainframe;
 define('_ACAJOOMCLASS', ACA_JPATH_ROOT . '/administrator/components/com_acajoom/classes/class.acajoom.php');
...
class getAcajoomTab extends cbTabHandler {
		  	...
		  	foreach ($lists as $list) {
				...
				if ($tabparams['show_archive'] && ($list->list_type == 1 or $list->list_type == 7)) {
					if ( ACA_CMSTYPE ) {	// joomla 15
						 $link = JRoute::_(ACA_JPATH_LIVE.'/index.php?option=com_acajoom&act=mailing&listid=' .$list->id . '&listype=' .$list->list_type .'&task=archive&Itemid=' . $item_id );
					} else {									//joomla 1x
	...
	function getEditTab( $tab, $user, $ui) {
		...
		require_once(ACA_JPATH_ROOT . '/administrator/components/com_acajoom/subscribers.acajoom.html.php');
		...
		if ( ACA_CMSTYPE ) {	// joomla 15
			...
			$forms['select'] = '<form method="post" action="'.JRoute::_( ACA_JPATH_LIVE.'/index.php?option=com_acajoom&act=subscriber').'"  name="AcajoomFilterForm">';
		} else {									//joomla 1x
