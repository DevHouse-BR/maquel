	...
	<?php
		if ($show['buttons']) {
			if($GLOBALS[ACA.'allow_unregistered'] OR $loggedin){
				...
				if ( ACA_CMSTYPE ) {	// joomla 15
						$link = JRoute::_(ACA_JPATH_LIVE.'/'.$show['index'].'.php?option=com_acajoom&act=subone&listid=' .$list->id . $item );
				} else {									//joomla 1x
			...
			if (($list->list_type == 1 or $list->list_type == 7) && $GLOBALS[ACA.'show_archive']=='1' ) {
				...
				if ( ACA_CMSTYPE ) {	// joomla 15
					$linkArchive = JRoute::_(ACA_JPATH_LIVE.'/'.$show['index'].'.php?option=com_acajoom&act=mailing&listid=' .$list->id . '&listype=' .$list->list_type .'&task=archive' . $item );
				} else {									//joomla 1x
					if ( function_exists('sefRelToAbs') AND $GLOBALS[ACA.'use_sef'] ) {
