		...
		switch ($task) {
			...
			default:
				...
				if ( ACA_CMSTYPE ) {	// joomla 15
					 $link = JRoute::_(ACA_JPATH_LIVE.'/index.php?option=com_acajoom&act='.$action );
				} else {									//joomla 1x
					$link = sefRelToAbs('index.php?option=com_acajoom&act='.$action);
				}//endif
		...
		switch ($task) {
			...
			case 'view':
				if ($mailingId != 0) {
					...
					if ( ACA_CMSTYPE ) {	// joomla 15
						 $linkZ = JRoute::_(ACA_JPATH_LIVE.'/index.php?option=com_acajoom' );
			...
			default:
				...
				if ( ACA_CMSTYPE ) {	// joomla 15
					 $linkZ = JRoute::_( ACA_JPATH_LIVE.'/'.$link );
				} else {									//joomla 1x
	...
	if ($GLOBALS[ACA.'require_confirmation']  AND $confirmation AND (!$confirm)) {
	 ...
	 function subscriptions($subscriberId, $listId, $action) {
		global $Itemid;
			...
			if ( ACA_CMSTYPE ) {	// joomla 15
				$forms['main'] = '<form method="post" action="'. JRoute::_(ACA_JPATH_LIVE.'/index.php?option=com_acajoom') . '" onsubmit="submitbutton();return false;" name="mosForm" >'."\n\r";
				$forms['select'] = '<form method="post" action="'. JRoute::_(ACA_JPATH_LIVE.'/index.php?option=com_acajoom&act='.$action) . '"  name="AcajoomFilterForm">';
			} else {									//joomla 1x
				if ( $GLOBALS[ACA.'use_sef'] AND function_exists('sefRelToAbs') ) {
				$forms['main'] = '<form method="post" action="'. sefRelToAbs('index.php?option=com_acajoom') . '" onsubmit="submitbutton();return false;" name="mosForm" >'."\n\r";
   ...
   return true;
	...
	function unsubscribe($subscriberId, $cle='', $listId, $action) {
	 ...
	 function showMailingsFront( $task, $action, $subscriberId, $listId, $listType, $viewArchive, $pageTile) {
		...
		if ( ACA_CMSTYPE ) {	// joomla 15
			$forms['main'] = '<form method="post" action="'. JRoute::_(ACA_JPATH_LIVE.'/index.php?option=com_acajoom&act=' . $action) . '" onsubmit="submitbutton();return false;" name="mosForm" >'."\n\r";
			$forms['select'] = '<form method="post" action="'. JRoute::_(ACA_JPATH_LIVE.'/index.php?option=com_acajoom&act=' . $action) . '"  name="AcajoomFilterForm">'."\n\r";
		} else {									//joomla 1x
			if ( $GLOBALS[ACA.'use_sef'] AND function_exists('sefRelToAbs') ) {
	 ...
	 function mailingEdit($subscriberId, $mailingId, $listId, $listType, $action) {
		...
		if ( ACA_CMSTYPE ) {	// joomla 15
			$forms['main'] = '<form method="post" enctype="multipart/form-data" action="'. JRoute::_(ACA_JPATH_LIVE.'/index.php?option=com_acajoom&act=savemailing') . '" onsubmit="submitbutton();return false;" name="adminForm" >'."\n\r";
		} else {									//joomla 1x
			if ( $GLOBALS[ACA.'use_sef'] AND function_exists('sefRelToAbs') ) {
