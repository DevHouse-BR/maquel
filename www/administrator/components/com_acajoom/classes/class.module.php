	...
	var $num = 0;
	 ...
	 function create() {
		...
		if (!empty($this->lists)) {
			...
			$h .= '<form action="'.$linkForm.'" method="post" class="modacajoom" name="modacajoomForm'.$this->num.'">
			<div class="newsletter'.$this->moduleclass_sfx.'" style="text-align:'.$this->mod_align.'">' ;

			if (!empty($this->introtext)) {
				$text = '<div class="pretext'. $this->moduleclass_sfx .'">'. $this->introtext .'</div>';
			...
			if ( $this->showListName ) {
			 ...
			 if (!$loggedin) {
				if ($GLOBALS[ACA.'allow_unregistered']) {
					 ...
					 if ($this->shownamefield) {
						$text =  'Nome:<br /><input id="wz_11" type="text" size="'. $this->fieldsize.'" class="inputbox" name="name" onblur="if(this.value==\'\') this.value=\''. addslashes(_ACA_NAME).'\';" onfocus="if(this.value==\''. addslashes(_ACA_NAME).'\') this.value=\'\' ; " />';
						$h .= acajoom::printLine($this->linear, $text);
					 } else {
						$text = '<input id="wz_11" type="hidden" value="" name="name" />';
					 }

					$text = 'Email:<br /><input id="wz_12" type="text" size="' .$this->fieldsize .'" class="inputbox" name="email" onblur="if(this.value==\'\') this.value=\'' . addslashes(_ACA_EMAIL) .'\';" onfocus="if(this.value==\'' . addslashes(_ACA_EMAIL) .'\') this.value=\'\' ; " /><br />';
					$h .= acajoom::printLine($this->linear, $text);
					...
					if ( isset( $GLOBALS[ACA.'cb_integration'] ) && $GLOBALS[ACA.'cb_integration'] ) {
						...
						if(ACA_CMSTYPE){//Joom1.5
							$linkme = 'index.php?option=com_user&amp;task=register';
						}else{
					...
					if ( ACA_CMSTYPE ) {	// joomla 15
						$text .= '<a href="'. JRoute::_( ACA_JPATH_LIVE.'/'.$linkme ).'">';
					} else {									//joomla 1x
			...
			if (!$loggedin) {
				...
				if ($GLOBALS[ACA.'allow_unregistered']) {
					...
					$text = '<input id="aca_22" type="button" value="Enviar" class="button" name="'.$this->buttonUnregistered.'" onclick="return submitacajoommod'.$this->num.'(\'modacajoomForm'.$this->num.'\');" />';
					$h .= acajoom::printLine($this->linear, $text);
					$h .= '
					</div>
				...
				if ( isset( $this->notifType ) AND $subscribed ) {
					...
					$text = '<input id="aca_22" type="image" src="'.$this->imgRegistered.'" value="'.$this->buttonRegistered.'" alt="'.$this->buttonRegistered.'" name="'.$this->buttonRegistered.'">';
					 ...
					 $text = '<input id="aca_22"  type="submit" value="Enviar" name="'.$this->buttonRegistered.'" class="button" />';
