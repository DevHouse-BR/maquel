...
/*		$content = str_replace('href="mailto:' , '9aca7aca5', $content);  // mailto tag good ones
		...
		if ( ACA_CMSTYPE ) {	// joomla 15
					$subscriptionslink = JRoute::_(ACA_JPATH_LIVE.'/index.php?option=com_acajoom&Itemid='.$Itemid.'&act=change&subscriber=' . $subscriber->id . '&cle=' . md5($subscriber->email). '&listid=' . $listId );
					$unsubscribelink = JRoute::_(ACA_JPATH_LIVE.'/index.php?option=com_acajoom&Itemid='.$Itemid.'&act=unsubscribe&subscriber=' . $subscriber->id . '&cle=' . md5($subscriber->email) . '&listid=' . $listId );
		} else {									//joomla 1x
			 if ($GLOBALS[ACA.'use_sef'] == '1' AND $GLOBALS['mosConfig_sef'] == '1' AND function_exists('sefRelToAbs')) {
					...
					$subscriptionslink = ACA_JPATH_LIVE.'/index.php?option=com_acajoom&Itemid='.$Itemid.'&act=change&subscriber=' . $subscriber->id . '&cle=' . md5($subscriber->email). '&listid=' . $listId  ;
					$unsubscribelink = ACA_JPATH_LIVE.'/index.php?option=com_acajoom&Itemid='.$Itemid.'&act=unsubscribe&subscriber=' . $subscriber->id . '&cle=' . md5($subscriber->email) . '&listid=' . $listId ;
			 }
		}//endif
		...
		if ( ACA_CMSTYPE ) {	// joomla 15
	  			$confirmlink = JRoute::_(ACA_JPATH_LIVE.'/index.php?option=com_acajoom&act=confirm&listid=' . $listId . '&cle=' . md5($subscriber->email) . '&subscriber=' . $subscriber->id.'&Itemid='.$Itemid);
		} else {									//joomla 1x
			if ( $GLOBALS['mosConfig_sef'] == '1' AND $GLOBALS[ACA.'use_sef'] =='1' AND function_exists('sefRelToAbs') ) {
	  			$confirmlink = sefRelToAbs('index.php?option=com_acajoom&act=confirm&listid=' . $listId . '&cle=' . md5($subscriber->email) . '&subscriber=' . $subscriber->id.'&Itemid='.$Itemid);
			} else {
	  			$confirmlink = ACA_JPATH_LIVE.'/index.php?option=com_acajoom&act=confirm&listid=' . $listId . '&cle=' . md5($subscriber->email) . '&subscriber=' . $subscriber->id.'&Itemid='.$Itemid;
			}
		}//endif
		...
		$replaceWhat = array('[CONFIRM]','[NAME]','[FIRSTNAME]','[EMAIL]','[DATE]','[USERNAME]');
 if ( ACA_CMSTYPE ) {
		$replaceBy = array($confirmlink,$subscriber->name,$firstname,$subscriber->email,JHTML::_('date',acajoom::getNow(), JText::_('DATE_FORMAT_LC1'), 0),$username);
 } else {
		$replaceBy = array($confirmlink,$subscriber->name,$firstname,$subscriber->email,mosFormatDate(acajoom::getNow(), '', 0),$username);
}
		...
		if ( !empty($mailingId) AND $GLOBALS[ACA.'enable_statistics'] == 1 ) {
			if ($GLOBALS[ACA.'statistics_per_subscriber'] == 1) {
  				...
  				if($html) $content .= '<img src="' . ACA_JPATH_LIVE . '/index.php?option=com_acajoom&Itemid='.$Itemid.'&act=log&listid=' . $listId . '&mailingid=' . $mailingId . '&subscriber=' . $subscriber->id . '" border="0" width="1" height="1" />';
			} else {

  				if ($html)	$content .=  '<img src="' . ACA_JPATH_LIVE . '/index.php?option=com_acajoom&Itemid='.$Itemid.'&act=log&listid=' . $listId . '&mailingid=' . $mailingId . '" border="0" width="1" height="1" />';
			}
		}
		// replace for images
		...
		$content = preg_replace('#src[ ]*=[ ]*\"(?!https?://)(?:\.\./|\./|/)?#','src="'.ACA_JPATH_LIVE.'/',$content);
		$content = preg_replace('#href[ ]*=[ ]*\"(?!https?://)(?:\.\./|\./|/)?#','href="'.ACA_JPATH_LIVE.'/',$content);
	 ...
	 function getMailer($mailing, $html=0) {
      ...
      $mail->addCustomHeader("X-MessageID: $mailing->id");
		...
		if(empty($mail->Sender)) {
			if ( ACA_CMSTYPE ) {	// joomla 15
				$conf	=& JFactory::getConfig();
				$mail->Sender = trim($conf->getValue('config.mailfrom'));
			} else {									//joomla 1x
	...
	function getContent($images, $layout, &$content, &$textonly, $send = false) {
	 ...
	 function send( $showHTML, $mailing, $receivers, $list, &$message, $tags=null ) {
		...
		foreach ($receivers as $receiver) {
			...
			$replaceWhat = array('[NAME]','[FIRSTNAME]','[USERNAME]');
			$replaceBy = array($receiver->name,$firstname,$username);
...
//ADRIEN REFRESH PAGE
	 ...
	 function sendOne($mailing, $receivers, $list, &$message , $tags=null) {
		...
		if(!empty($receivers)){
			...
			$replaceWhat = array('[NAME]','[FIRSTNAME]','[USERNAME]');
			$replaceBy = array($receivers->name,$firstname,$username);
			$sujetReplaced = str_replace($replaceWhat, $replaceBy, $subject);
			$mail->Subject =  $sujetReplaced;
		...
		if (lisType::sendLogs($list->list_type)) {
		 ...
		 if ($GLOBALS[ACA.'save_log']) {
			 ...
			 if ($GLOBALS[ACA.'save_log_simple']) {
				 @file_put_contents(ACA_JPATH_ROOT . $GLOBALS[ACA.'save_log_file'], $log_simple, FILE_APPEND);
			 } else {
				 @file_put_contents(ACA_JPATH_ROOT . $GLOBALS[ACA.'save_log_file'], $log_detailed, FILE_APPEND);
			 }
		 }
