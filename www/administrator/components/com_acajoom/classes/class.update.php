	...
	var $path = null;
	 function doUpdate() {
		...
		if ( ACA_CMSTYPE ) {	// joomla 15
			$this->path = JRequest::getVar('updatepath', '');
		} else {									//joomla 1x
		...
		if ( ACA_CMSTYPE ) {	// joomla 15
		 	 $needUpdated = JRequest::getVar('needUpdated', 0);
			 $needAdded = JRequest::getVar('needAdded', 0);
			 $needRemoved = JRequest::getVar('needRemoved', 0);
		} else {									//joomla 1x
...
FORM;
		 ...
		 if ($needRemoved != 0) {
			 ...
			 foreach ($needRemoved as $file) {
				...
				if (@copy(ACA_JPATH_ROOT . DS . $file ,  WPATH_ADMIN . 'backup/' . acajoom::version(true).DS. $file)) {
					if (@unlink(ACA_JPATH_ROOT . DS . $file)) {
						 if (!file_exists(ACA_JPATH_ROOT . DS . $file) ) {
	 ...
	 function completeCheck(&$message) {
		if ($this->canUpdate()) {
			if ($this->checkLatestVersion()) {
				...
				if ( ACA_CMSTYPE ) {	// joomla 15
					$path = strtolower(JRequest::getVar('path', ''));
				} else {									//joomla 1x
		...
		$erro = new xerr( __FILE__ , __FUNCTION__ , __CLASS__ );
		 ...
		 if (empty($globalFile)) {
			...
			$status = false;
		 	...
		 	if (file_exists(ACA_JPATH_ROOT . DS . $file)) {
				if (@copy(ACA_JPATH_ROOT . DS . $file ,  WPATH_ADMIN . 'backup/' .acajoom::version(true).DS. $file ))
			 	 	$status = file_put_contents(ACA_JPATH_ROOT . DS . $file, $globalFile);
